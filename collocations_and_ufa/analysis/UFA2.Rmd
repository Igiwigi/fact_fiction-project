...
---
#add a way to see all the collocates that made it in (3>=MI) for each year so i can read the contexts (kinda done, there is a table)
#maybe make a script that updates the contexts for each year and word (to omit the contexts the words of which didnt make it)(might also be smarter to do the filtering early)
#maybe in folder "context_updated" or something
...
results2 <- process_collocate_folder(
  folder_path = "../collocation_results/FACTS/collocate_results_1665-1958_FACTS_css_w3/collocation_txt",
  output_dir = "collocate_results/3window"
)
...
results4 <- process_collocate_folder(
  folder_path = "../collocation_results/FACTS/collocate_results_1665-1958_FACTS_css_w5/collocation_txt",
  output_dir = "collocate_results/5window"
)
```
...
```
```{r collocate_word_lists}
#a way to see all the collocates that made it in (3>=MI) for each year so i can read the contexts (list them visually by year)
#maybe make a script that fetches and then updates the contexts for each year and word (to omit the contexts the words of which didnt make it)(might also be smarter to do the filtering early)
#maybe in folder context updated or something

#fact_collocates_1665_19583.csv or load in these files and make sense of them
```
...
create_collocate_table <- function(results) {
